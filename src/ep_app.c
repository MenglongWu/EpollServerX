...
void PrintConnectRemoveInf(struct ep_con_t *pconNode, int flag)
{
	...
	// fd = tms_GetManageFd();
	printf("%s", strout);
	tms_Trace(NULL, strout, offset + 1, 1);
}
...
{
	{"frame","display dev frame", NULL},
	{"connect","display dev connect", NULL},
	{"all","display all", NULL},
};
...
struct cmd_prompt cmd_olplist[] = 
{
	...
	{"disp","display something", cmd_displist},
	{"remote","run shell on remote",cmd_olplist},
	{NULL,NULL, NULL},
};
